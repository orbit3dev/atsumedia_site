...
const EditModal = modal<CategoryTableType>(({ onClose, data }) => {
	...
	const { mutate: addTodo, isLoading: addLoading } = useMutation<CategoryTableType>(
		({ id, name }) =>
			userPoolClient.models.Category.create({ id, name, sort: parseInt(id), type: TableName.Category }),
		{
			successMessage: 'Categoryを追加しました。',
		}
	);

	const { mutate: updateTodo, isLoading: updateLoading } = useMutation<CategoryTableType>(
		({ name }) => userPoolClient.models.Category.update({ id: data.id, name }),
		{
			successMessage: 'Categoryを更新しました。',
		}
	);

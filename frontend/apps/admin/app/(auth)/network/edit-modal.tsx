...
import { NetworkType } from '@admin/(auth)/network/page';
...
const EditModal = modal<NetworkType>(({ onClose, data }) => {
	...
	const { mutate: addTodo, isLoading: addLoading } = useMutation<NetworkType>(
		({ id, name }) =>
			userPoolClient.models.Network.create({ id, name, sort: parseInt(id), type: TableName.Network }),
		{
			successMessage: 'Networkを追加しました。',
		}
	);

	const { mutate: updateTodo, isLoading: updateLoading } = useMutation<NetworkType>(
		({ name }) => userPoolClient.models.Network.update({ id: data.id, name }),
		{
			successMessage: 'Networkを更新しました。',
		}
	);

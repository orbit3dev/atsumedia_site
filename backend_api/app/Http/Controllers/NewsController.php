...
class NewsController extends Controller
{
        ...
        foreach ($lines as $index => $rawRow) {
            ...
            try {
                ...
                $articleFreeText = [
                    ...
                    'free_text_id' => 0,
                    'content' => $this->transformContentArticle($rowData['content'], $baseImagePath, '_article_', $article_id, $pathEnv),
                    'created_at' => $createdAt,
                    'updated_at' => $updatedAt,
                ];
                AtArticleFreeText::create($articleFreeText);
            } catch (\Exception $e) {
                Log::error("Error processing row: " . $e->getMessage());
            }
        }
    ...
    private function transformContentArticle(string $contentJson, string $basePath, string $type, string $article_id, string $pathEnv): string
    {
        $rawJson = substr($contentJson, 3, -3);
        $dataJson = str_replace('""""', '"', $rawJson);
        preg_match_all(
        ...
        foreach ($foundUrls as $url) {
            try {
                ...
                $image_link = !empty(env('IMAGE_LINK')) ? env('IMAGE_LINK') : $pathEnv;
                $url = str_replace($baseS3Url, '', $url);
                $fileName = $this->downloadAssetArticle($url, $basePath, 'content_image', $article_id);
                if ($fileName) {
                    $dataJson = str_replace($baseS3Url . $url, $image_link . 'news/content/image/' . $fileName, $dataJson);
                }
            } catch (\Exception $e) {
    ...
    private function downloadAssetArticle(string $url, string $basePath, string $type, string $articleId): ?string
    {
        if (empty($url)) {
            Log::info("Empty URL provided for {$type}");
            return null;
        ...
        $existingFiles = glob($basePath . DIRECTORY_SEPARATOR . "{$articleId}_article_*.{jpg,jpeg,png,webp,gif}", GLOB_BRACE);
        $picNumber = count($existingFiles) + 1;
        $fileName = "{$articleId}_article_{$picNumber}.{$extension}";
        $fullPath = $basePath . DIRECTORY_SEPARATOR . $fileName;
